...
class RecordingApifoxClient:
    ...
    def list_http_apis(self, project_id: str) -> list[dict[str, object]]:
        return [
            {"id": 101, "method": "post", "path": "/orders", "moduleId": 303},
            {"id": 202, "method": "get", "path": "/orders/{id}", "moduleId": 303},
            {"id": 909, "method": "post", "path": "/unrelated", "moduleId": 303},
        ]
...
def test_api_mapper_agent_declares_apifox_source_without_token():
    declaration = ApiMapperAgent().declare_api_source(
    ...
    assert "token" not in declaration
    assert "Authorization" not in json.dumps(declaration)
...
def test_apifox_client_builds_selected_endpoint_export_request():
    ...
    request_spec = client.build_export_openapi_request(
        project_id="123456",
        endpoint_ids=[101, 202],
        module_id=303,
    )

    assert request_spec.body["scope"] == {
        "type": "SELECTED_ENDPOINTS",
        "selectedEndpointIds": [101, 202],
    }
...
def test_api_document_resolver_fetches_only_matched_endpoint_details():
    ...
    resolver = ApiDocumentResolver(
        credential_provider=FakeCredentialProvider(),
        apifox_client_factory=make_client,
    )
